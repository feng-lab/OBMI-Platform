...
def extraction_v3(cell_mask, bg_mask, params, img):
    '''
    :param contours: Contrours of ROI
    :param params: Bounding box of ROI: [x, y, width, height]
    :param img: Frame
    ...
    if x_max > img.shape[1] - 1:
        x_max = img.shape[1] - 1
        bg_mask = bg_mask[:, :-(x_max - img.shape[1] + 1)]
    if y_min < 0:
        y_min = 0
        bg_mask = bg_mask[-y_min:, :]
    if y_max > img.shape[0] - 1:
        y_max = img.shape[0] - 1
        bg_mask = bg_mask[:-(y_max - img.shape[0] + 1), :]
...
def extract_process(input_stream:Queue, output_stream:Queue, params, masks, bg_masks):
    while True:
