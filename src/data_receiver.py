...
class ReceiverThread(QThread):
    ...
    def run(self):
        ...
        import threading
        print("receive thread:", threading.get_ident())
        et = 0
        while not self.isInterruptionRequested():
            if self.img_buffer.empty():
                if not self.parent.rt:
                    return
                self.interval += 0.005
                time.sleep(0.01)
                continue
            if et != 0:
                print('loop interval:', time.time()-et)
            img = self.img_buffer.get()
            print('read img, remaining: ', self.img_buffer.qsize())
            t0 = time.time()
