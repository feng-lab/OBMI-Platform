...
class OPlayerObject(QObject):
    ...
    def startCapture(self):
        ...
        self.frameI.connect(self.main_win.online_frame)
        ...
        self.frameI.disconnect(self.main_win.online_frame)
        capture.release()  ##
...
class OPlayer(QtCore.QThread):
    ...
    def run(self):
        ...
        self.player_object = OPlayerObject(capture, self.data_lock, self.parent)
        self.player_object.moveToThread(self)
        self.player_object.startCapture()
        # self.exposure = int(capture.get(cv2.CAP_PROP_EXPOSURE))
        # self.s_gain = int(capture.get(cv2.CAP_PROP_GAIN))
        # self.hue_value = 0
        #
    ...
    def set_fps(self):
        self.player_object.set_fps()

    def cal_FPS(self, dur):
        self.player_object.cal_FPS(dur)

    def get_fps(self, cap: cv2.VideoCapture):
        self.player_object.get_fps(cap)
