...
## hwabt par처리
    def __init__(self, camera: int, cam_type: str, lock: QtCore.QMutex, parent: QtCore.QObject, 
                    user_path: str, f_type: str, pj_name: str): 
        super().__init__(parent=parent) 

        self.camera_ID = camera
        self.cam_type = cam_type # 'B', 'S'
        self.data_lock = lock
        self.base_path = user_path # + cam_type
        ...
        self.hue_control_value = self.hue_status
        ## --------------
        self.tfps = 0
    ...
    def run(self):
        ...
        self.hue_status = cap.get(cv2.CAP_PROP_HUE)
        print('hue_status: ', self.hue_status)
        self.hue_value = 0 ###
        '''
        ##
        ...
        if self.cam_type == 'S': ## self.camera_ID == cv2.CAP_DSHOW + 0:
        ...
        while not self.isInterruptionRequested(): ## 기능나누기|같이쓰기. scope behavier
            ...
            if self.exposure_status != self.exposure_control_value:
                cap.set(cv2.CAP_PROP_BRIGHTNESS, self.exposure_control_value)
                print('exposure_control_value: ', self.exposure_control_value) #check
                self.exposure_control_value = cap.get(cv2.CAP_PROP_BRIGHTNESS) #check
                print('exposure_controled_value: ', self.exposure_control_value) #check
                self.exposure_status = self.exposure_control_value
            if self.gain_status != self.gain_control_value:
                self.change_gain(cap, self.gain_control_value)
                print('gain_control_value: ', self.gain_control_value) #check
                self.gain_control_value = int(cap.get(cv2.CAP_PROP_GAIN))
                print('gain_controled_value: ', self.gain_control_value) #check
                self.gain_status = self.gain_control_value
            if self.hue_status != self.hue_control_value:
                hue = self.change_led(cap, self.hue_control_value)
                cap.set(cv2.CAP_PROP_HUE,(hue>>4)&0x00FF)
                print('hue_control_value: ', self.hue_control_value) #check
                self.hue_control_value = cap.get(cv2.CAP_PROP_HUE)
                print('hue_controled_value: ', self.hue_control_value) #check
                self.hue_status = self.hue_control_value
      ...
      #         time.sleep(delay)
            ...
            ## para or h ordr
            pres_time = time.time()
            fps = round(1/(pres_time-prev_time),3) 
