...
class NCCProject():
    ...
    def __init__(self, template_frame, path):
        ...
        self.kernel=torch.tensor(self.LOAD.generate_kernel((KERNEL_SIZE,KERNEL_SIZE)), dtype=torch.float32)#kernel尺寸可能后期要根据神经元大小调整
...
class Preprocess(sig):
    ...
    def generate_template(self):
        ...
        LOAD=ld(1)
        # 1 first frame
        kernel=torch.tensor(LOAD.generate_kernel((KERNEL_SIZE,KERNEL_SIZE)), dtype=torch.float32)
