...
import numpy as np
import cv2
from src2.offline.mcc.compute_offset import ApplyShifts
from src2.offline.mcc.LoadData import load_data as ld
from src2.offline.mcc.correlation_torch import NormXCorr2
...
class on_NCC(): #H
    ...
    def __init__(self, template_frame, init_frame):
        ...
        self.kernel=torch.tensor(self.LOAD.generate_kernel((7,7)), dtype=torch.float32)#kernel尺寸可能后期要根据神经元大小调整
        self.sum1, self.a_rot_complex, self.b_complex, self.Zeros, self.theta, self.template_buffer = self.LOAD.SetParameters(
            raw_frame, self.kernel, CROP=False, use_gpu=True)
        self.kernel=self.kernel.cuda()
        self.ith=0
